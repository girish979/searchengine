/*
Edited, takes a string as input
...
extern char g_sterm[140];

void stemfile(struct stemmer * z, char f[],int len)//edited //FILE * f)
{  
	int temp = 0;//edited
	f[len]='\0';
   while(temp < len)
   {  int ch = f[temp++];
      ...
      {  int i = 0;
         ...
         {  if (i == i_max)
            ...
            // eidted; ch = getc(f);
	    ch = f[temp++];
            if (!LETTER(ch) ) {  temp--;break; } //edited; { ungetc(ch,f); break; }
         }
         s[stem(z, s, i - 1) + 1] = 0;
...
{  int i;
   ...
   s = (char *) malloc(i_max + 1);
	...
	len = strlen(argv);//edited  //Read 1st word from input
   stemfile(z,argv,len); //f file pointer //Edited by Me
